...
xgb_control_exp <- trainControl(method = "adaptive_cv",
                            number = 2,
                            repeats = 2,
                            search = "random",
                            summaryFunction = twoClassSummary,
...
for (set in splits) {
  ...
  perf_auc <- performance(prediction(pred_ad_test_boost_exp[,2], autodata_exp_test$Target), "auc")
  auc <- unlist(slot(perf_ad_test_boost_auc_exp, "y.values"))
