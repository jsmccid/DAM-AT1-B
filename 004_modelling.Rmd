...
# long tune
# ROC was used to select the optimal model using the largest value.
# The final values used for the model were nrounds = 570, max_depth
#  = 10, eta = 0.2089189, gamma = 7.282036, colsample_bytree
#  = 0.3903721, min_child_weight = 0 and subsample = 0.9241232.

# longer tune

  test_x <- model.matrix(~. -ID + annualised_mileage*age_of_vehicle_years, autodata_test[, -2])
  test_y <- autodata_test$Target

  pred_prob <- predict(autodata_xgbmodel_adapt, test_x, type = "prob")

  pred_class <- predict(autodata_xgbmodel_adapt, test_x, type = "raw")
  ...
  # save all outputs
  listout <- list()
  listout <- llist(autodata_xgbmodel_adapt, end_time, pred, confusionm, perf, auc)
  
  assign("xbg_long_tune", listout)
  save(xbg_long_tune, file = "./models/longtuneboost.Rdata")
```
