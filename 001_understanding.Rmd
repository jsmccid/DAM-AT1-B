...
autodata %>% 
  filter(Target == 1) %>% 
    ggplot(aes(x = car_model)) + geom_bar() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
autodata %>% 
  group_by(car_model) %>% 
  summarise(nonshed = sum(non_sched_serv_warr)) %>% 
ggplot(aes(x = car_model, y = nonshed)) + geom_col() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

```{r}
#most miles
ggplot(autodata, aes(x = car_model, y = total_km)) + geom_box() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
